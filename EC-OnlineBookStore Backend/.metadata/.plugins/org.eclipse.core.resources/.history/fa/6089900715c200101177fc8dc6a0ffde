...
//	@GetMapping("/getallratingsbyid/{id}")
//	public ResponseEntity<ResponseMessage> getAllRatings(@PathVariable Long id){
//		List<Ratings> byBookId = ratingService.getByBookId(id);
//		if(byBookId!=null) {
//			return ResponseEntity.ok(new ResponseMessage(HttpURLConnection.HTTP_OK, Constants.SUCCESS, "custmer  getting all reviews successfully", byBookId));
// 		}else {
// 			return ResponseEntity.ok(new ResponseMessage(HttpURLConnection.HTTP_BAD_REQUEST, Constants.FAILED, "custmer  getting all reviews Failed", byBookId));
//		}
//	}
}
